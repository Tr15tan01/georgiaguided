...
export function MediaDialog({ open, onClose, onSelect, multiple }: { open: boolean; onClose: () => void; onSelect: (items: MediaItem[]) => void; multiple?: boolean }) {
  ...
  useEffect(() => {
    const d = ref.current;
    if (!d) return;
    if (open && !d.open) { d.showModal(); setSelected([]); load(q, page); }
      ...
      <div className="flex h-full flex-col">
        <div className="flex flex-wrap items-center gap-3 border-b border-line p-4">
          ...
          <label className={cn("adm-btn adm-btn-primary cursor-pointer", (!configured || uploading) && "pointer-events-none opacity-60")}>
            ...
            <input type="file" accept="image/jpeg,image/png,image/webp,image/avif,image/gif" multiple className="sr-only" onChange={(e) => upload(e.target.files)} />
          </label>
              ...
              {items.map((m) => (
                <li key={m._id}>
                  <button type="button" onClick={() => toggle(m._id)} aria-pressed={selected.includes(m._id)} className={cn("relative block aspect-square w-full overflow-hidden rounded-md border-2", selected.includes(m._id) ? "border-accent" : "border-transparent")}>
                    <Image src={m.url} alt={m.alt} fill sizes="160px" className="object-cover" unoptimized={m.url.endsWith(".svg")} />
                  </button>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-line p-4">
          <div className="flex items-center gap-2 text-sm">
            <button type="button" className="adm-btn" disabled={page <= 1} onClick={() => { setPage(page - 1); }}>Previous</button>
            <span className="text-ink-soft">Page {page} of {pages}</span>
            <button type="button" className="adm-btn" disabled={page >= pages} onClick={() => { setPage(page + 1); }}>Next</button>
          </div>
          <button type="button" className="adm-btn adm-btn-primary" disabled={!selected.length} onClick={() => { onSelect(selected.map((id) => items.find((i) => i._id === id)!).filter(Boolean)); onClose(); }}>
      ...
      {value?.url ? (
        <div className="flex flex-col gap-3 rounded-lg border border-line p-3 sm:flex-row">
          <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden rounded-md bg-line sm:w-48">
            <Image src={value.url} alt={value.alt} fill sizes="200px" className="object-cover" unoptimized={value.url.endsWith(".svg")} />
          </div>
